...
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/components/make/mobile.socialnetwork.log.ex/include.php");
...
if ($arParams["EMPTY_PAGE"] != "Y")
{
    ...
    // get user's groups and set ID based on those
    $sites = [];
    $userGroups = $USER->GetUserGroupArray();
    foreach ($userGroups as $key => $groupId) {
        $siteId = userSiteByGroupId($groupId);
    ...
    // SUPPORT-1168 (fix issue with activity streams per site/division in mobile app)
