...
class LogList extends \CBitrixComponent implements \Bitrix\Main\Engine\Contract\Controllerable, \Bitrix\Main\Errorable
{
	...
	protected function getEntryIdList(&$result)
	{
		...
		$snFilter = $processorInstance->getFilter();
		global $USER;
		$permUG = $USER->GetUserGroup($USER->GetID());

		if($snFilter['SITE_ID'] == "s1" && in_array(12, $permUG)){


			$snFilter['SITE_ID'] = [];
			$sitePerms = [
				12 => 's1', // ddb/ main stream
                17 => 'mh', // sts biler
                19 => 'lb', // era biler
                21 => 'py', // mfa biler
                23 => 'rc', // bil og co
            ...
            foreach ($permUG as  $value) {

            	if(array_key_exists($value, $sitePerms)){
            		$snFilter['SITE_ID'][] =  $sitePerms[$value];
            	}
            }
		}
