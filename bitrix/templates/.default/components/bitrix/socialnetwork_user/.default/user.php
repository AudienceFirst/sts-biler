...
if (\Bitrix\Main\ModuleManager::isModuleInstalled("intranet") && SITE_TEMPLATE_ID == "bitrix24")
{
	...
	else
	{
		...
		{
			$isIntranetUser = CExtranet::IsIntranetUser(SITE_ID, $arResult["VARIABLES"]["user_id"]);

			$intranetSearchComponentParams["LIST_URL"] = (
				$isIntranetUser
					? SITE_DIR.'contacts/employees.php'
					: SITE_DIR.'contacts/'
			);
			$intranetSearchComponentParams["FILTER_NAME"] = "contacts_search";
			$intranetSearchComponentParams["STRUCTURE_FILTER"] = "contacts";
			$intranetSearchComponentParams["EXTRANET_TYPE"] = (
				$isIntranetUser
					? "employees"
					: ""
			);
		...
		$APPLICATION->IncludeComponent("make:intranet.search", "bitrix24", $intranetSearchComponentParams);
	}
}
else
{
